...
esp_err_t app_api_auth_change_password(httpd_req_t *req)
{
  ...
  if (remaining >= sizeof(buf)) return API_RETURN_ERR(req, 400, ESP_ERR_NO_MEM);

  ret = httpd_req_recv(req, buf, remaining);
  if (ret <= 0)
  {
    if (ret == HTTPD_SOCK_ERR_TIMEOUT) return API_RETURN_ERR(req, 408, ESP_ERR_TIMEOUT);
  ...
  {
    return API_RETURN_ERR(req, 400, ESP_ERR_INVALID_ARG);
  }

  esp_err_t err = nvs_manager_load_str(AUTH_NVS_NS, AUTH_NVS_KEY, buf, sizeof(buf));
  if (err != ESP_OK) return API_RETURN_ERR(req, 500, err);
  ...
  {
    return API_RETURN_ERR(req, 403, APP_ERR_INVALID_PASSWORD);
  }
